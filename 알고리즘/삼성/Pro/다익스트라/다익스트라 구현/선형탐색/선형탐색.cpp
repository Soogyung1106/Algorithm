...
int dp[6]; //최단 거리 dp 저장, 1에서 i까지 가는 최단거리 저장
int graph[6][6] = { //전체 그래프를 초기화합니다. 
	{0, 2, 5, 1, INF, INF},
	{2, 0, 3, 2, INF, INF},
	{5, 3, 0, 3, 1, 5},
	{1, 2, 3, 0, 1, INF},
...
void dijkstra(int start) {
	...
	for (int i = 0; i < number - 2; i++) {
		...
		for (int j = 0; j < 6; j++) {
			//★거쳐서 가는 비용이 더 작다면★
			if (!visited[j]) {
				if (dp[current] + graph[current][j] < dp[j])
					dp[j] = dp[current] + graph[current][j];//최소비용으로 갱신
			}
		}
	}
}
...
int main() {
	...
	dijkstra(0); //0(1)번 노드부터 모든 노드까지의 최단거리 
	for (int i = 0; i < number; i++) 
		cout << dp[i]<<" ";
